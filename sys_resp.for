...
c      write(6,*) stat,comp,time_event                                                                         
      call find_resp_file(stat,comp,time_event,cal_file,istart)
...
c read number of zeros
      ...
      if (line(1:5).eq.'ZEROS') then
        ...
        do x=1,nzero
          ...
          zero_cnt=zero_cnt+1
          zero(zero_cnt)=cmplx(re,im)
          write(*,*) zero_cnt,zero(zero_cnt)
        enddo
      endif
c read poles
c      read(read1,'(a)',end=999) line
99    continue
...
c------------------------------------------------------------------------------------------
      ...
      subroutine find_resp_file(stat,comp,time_event,cal_file,istart)
c
c routine searches for path and filename to CAL file
c    
c      input is station,stat, component,comp, time of event, time_event
...
c
c-----------------------------------------------------------------------
c seed response file, e.g. RESP.GE.DSB..HHZ, in local working directory
c-----------------------------------------------------------------------
c
c   make directory name and start of file name for that station and component
c
      filename='.'//dchar//'RESP.'
c
c   read all seed response files 
c
      call respfilseed(filename,7,cal_files,
     *max_cal_files$,ncal_files)
      
c      write(6,*) ' number of seed calibration files', ncal_files

      if(ncal_files.gt.0) then
         ical=0
         do i=1,ncal_files
c           write(6,*)cal_files(i)
c           write(6,*)cal_files(i)(9:8+seiclen(wav_stat
c     *             (wav_current_chan(1))))
c     *              ,wav_stat(wav_current_chan(1))
c     *              (1:seiclen(wav_stat(wav_current_chan(1))))
c     *              ,cal_files(i)(seiclen(cal_files(i))-2:
c     *              seiclen(cal_files(i)))
c     *              ,wav_comp(wav_current_chan(1))(1:2)//
c     *              wav_comp(wav_current_chan(1))(4:4)

           seed_comp=comp
           do j=1,seiclen(seed_comp)
              if (seed_comp(j:j).eq.' ')seed_comp(j:j)='_'
           enddo
c          write(*,*) ' xxx ',cal_files(i),
c     &    wav_stat(wav_current_chan(1)),seed_comp

           if (cal_files(i)(9:8+seiclen(stat))
     &     .eq.stat(1:seiclen(stat)) 
     &     .and.cal_files(i)(seiclen(cal_files(i))-2:
     &     seiclen(cal_files(i)))
     &     .eq.seed_comp(1:2)//seed_comp(4:4)) then
              cal_file=filename(1:k+5)//cal_files(i)
     &        (1:seiclen(cal_files(i)))
              ical=i
...
c
c-----------------------------------------------------------------------
c seed response file, e.g. RESP.GE.DSB..HHZ, in cal directory
c-----------------------------------------------------------------------
c
c   make directory name and start of file name for that station and component
c
      filename=
     *top_directory(1:k)//dchar//'CAL'//dchar//'RESP.'
c
c   read all seed response files 
c
      call respfilseed(filename,k+10,cal_files,
     *max_cal_files$,ncal_files)
...
c        read(line(16:43),'(2e14.2)') x,y
      ...
      elseif (line(1:7).eq.'B058F04'.and.flag) then
        if (seed_gain(stage).eq.0.) read(line(52:),*) seed_gain(stage)
c decimation
...
c        write(*,*) ' stage 0 '
        flag=.false.   
      elseif (line(1:7).eq.'B058F04'.and..not.flag) then
...
c          write(*,*) ' seed_gain ',seed_gain(i)
          norm=norm*seed_gain(i)
        endif
        if (a0_norm(i).ne.0.) then 
c          write(*,*) ' a0_norm ',a0_norm(i)
          norm=norm*a0_norm(i)
        endif
        do j=1,seed_np(i)
          npol=npol+1  
