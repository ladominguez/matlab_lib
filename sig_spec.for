...
c 9 04  2010 jh:  Moved out again since thsi gave problem in spec so
...
c        ntap = (nsamp/2) * (taper/100)                                          
c        arg = pi / 2 / ntap                                                     
c        do i = 1,ntap                                                           
c         y(i) = y(i) * sin((i-1) * arg)                                   
c         y(nsamp-i+1) = y(nsamp-i+1) * sin((i-1) * arg)                   
c        enddo                                                                   
      endif                                                                     
c                                                                               
...
c
c   zero butterworth filter, jh jan 19 2010, was doen in connection
c   with new magntudes and correction for filter in amgnitude amplitude.
c   however, made problems fo rother programs like spec, so now it si
c   move outside agaibn, so far on ly zereos when used in connection
c   with mulplt
c
c     pole_low=0
c     pole_high=0
