...
c
c     write(6,*)' pv: eev_sub :  249: end time : ',end_time
 10   continue
...
c                                                                               
      ...
      if(keys(1:3).eq.'tsd') keys(1:3)='TSD'
...
c
         call read_s_file_e(evfile)
c
c  check if any request lines, i fso registration not allowed
c
         request_flag=.false.
         do i=2,nrecord1
...
c------------------------------------------------------------------------
c gmap
c tested with Firefox peter voss 11 OCT 2007
c------------------------------------------------------------------------
...
c
     ...
     *      (write01,nstat1,nphase1,nhead1,nrecord1,evid1,exp1,data1,id)
         ...
         write(*,*)"Location is generated for maps.google.com"
c        write(*,*)"          open gmap.html with you browser"
     ...
     &                       code)                  ! Returned condition
           ...
           txt(41:78)='url=http://www.maps.google.com/maps?q='
           i=79
...
c
           ...
           k=sei clen(gmap_dir)
           if(.not.pc)
     *     call systemc("cp gmap.html "//gmap_dir(1:k),13+k)
           if(pc)
     *     call systemc("copy gmap.html "//gmap_dir(1:k),15+k)
           endif

           write(*,*)' Open gmap.html with your favorite browser'
           if(gmap_dir.eq.' ') then
             write(6,*) ' you will find it in working directory'
           else
              write(*,*)' you will find it here : ',gmap_dir(1:k)
           endif
            keys(1:4)='SAME'
            goto 10
...
c----------------------------------------------------------
c
      if(keys(1:2).eq.'F '.or.keys(1:2).eq.'FO') then
c
c   first locate event, do not write out
c
         if(keys(2:2).eq.'O') answer='o'  ! no question, plot saved solution
         call hyp_loc(answer)
         if(answer.eq.'N') then
            keys(1:4)='SAME'
            goto 10
     ...
     *             nrecord1,evid1,exp1,data1,id)
           ...
           do i=2,nhead1
             if(data1(i)(80:80).eq.'6'.and.data1(i)(2:2).ne.' ') then
                wave_file=' '
                wave_file(1:78)=data1(i)(2:79)
                if(wave_file(1:3).ne.'BUD'.or.wave_file(1:3).ne.'SCP') 
     *          then
                   call get_full_wav_name(wave_file,text)
               else
                   karc=karc+1
...
c          k=0
c          do i=2,nhead1
c            if(data1(i)(80:80).eq.'6'.and.data1(i)(2:2).ne.' ') then
c               wave_file=' '
c               wave_file(1:78)=data1(i)(2:79)
c               call get_full_wav_name(wave_file,text)
c               if(text.ne.' ') then
c                 k=k+1
c               endif
c            endif
