...
c-------------------------------------------
c
      if(file_in(1:3).eq.'BUD'.or.file_in(1:3).eq.'SCP') then
c
...
c
         arc_type=0   ! bud
         if(file_in(1:3).eq.'SCP') arc_type=1  ! seiscomp
c
...
c
         do i=1,n_wav_bases
c
c   old seisn file name without century
c
...
c
              ...
              if (parameter.eq.1.) cont_base_def(n_cont_base)=1
            endif
c
...
c            if(parameter.gt.0.0) cont_int_s=parameter
     ...
     *     .and.data(51:60).ne.'          ') then
         ...
         elseif(data(1:8).eq.'ARC_CHAN') then
            iarc=iarc+1
            read(data(41:120),'(a5,a3,2a2)')arc_stat(iarc),
     *      arc_comp(iarc),arc_net(iarc),arc_loc(iarc)
         elseif(data(1:11).eq.'ARC_ARCHIVE') then
