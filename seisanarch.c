...
char *getArchFilename(hptime_t time, char *filename, char *af, char *sta, char *chan, char *net, char *loc, int *archiveType);
...
float *nsec;
{
	...
	*t_error = 0;
	...
	getArchFilename(starttime, fstart, tarch, tstat, tchan, tnet, tloc, archiveType);
	getArchFilename(endtime, fend, tarch, tstat, tchan, tnet, tloc, archiveType);
	//printf("Filenames:\n <%s>\n <%s>\n", fstart, fend); //For debugging
	...
	*samprate = (float)mstg->traces->samprate;
	...
	//For debugging
	//printf("XXXXXXX %2d:%2d:%6.3f XXXXXXX\n", *nhour, *nmin, *nsec);
...
int *samples;		//Array with the requested samples
{
	...
	//Print samples in ASCII format for testing
	/*
	FILE * pFile;
	int n;
   	char testfilename [100];
	double testtime = (double)starttime;
        sprintf(testfilename, "/home/seismo/WOR/%s_%s.txt", tstat, tchan);
   	pFile = fopen (testfilename,"w");
   	for (n=0 ; n<nsamples ; n++)
   	{
     		fprintf (pFile, "%14f %10d\n",testtime, samples[n]);
		testtime += 10000; //Equivalent to 100 sps
   	}
   	fclose (pFile);
	*/	
...
*/
char *getArchFilename(hptime_t time, char *filename, char *af, char *stat, char *chan, char *net, char *loc, int *archiveType)
{
	BTime btime;
	...
	else if ( *archiveType == 1 )
	{
		...
		sprintf(filename, "%s/%d/%s/%s/%s.D/%s.%s.%s.%s.D.%d.%03d", 
			af, btime.year, net, stat, chan, net, stat, loc, chan, btime.year, btime.day);
	}
	else 
	{
	...
	for (fcount = 0; fcount<nFiles; fcount++)
	{
		...
		while ((retcode = ms_readmsr(&msr, curFilename, 0, NULL, NULL, 1, dataflag, 0)) == MS_NOERROR)
		{
			...
			if ((recstart<=endtime) && (recend >= starttime))
			{
				...
				//Check time correction only for the first record
				/*				
				if ((get_time_correction) && ((msr->fsdh->io_flags&128)>>7))
				{
					*t_error = msr->fsdh->time_correct;
