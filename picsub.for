...
c                      filter info and give it color, initilize variable ch
...
c                                                                               
c   check if filter possible in time domain, 
c   check not needed  if remove response or spectrum, since 
...
c  use filter
c
c           write(6,*) 'ml filt poles',ml_low_pole,ml_high_pole
            if(ml_low_pole.gt.0.or.ml_high_pole.gt.0) then
...
c
          if(do_mb) then
             ...
             disp_vel=1
c
c  use filters, disabled for now
c
c            write(6,*) 'mb filt poles', mb_low_pole,mb_high_pole
c             if(mb_low_pole.gt.0.or.mb_high_pole.gt.0) then
c               write(6,*) 'in mb filt'
c                 filt=9
c                 flow(9)=mb_low
c                 flow_pole(9)=mb_low_pole
c                 fhigh(9)=mb_high
c                 fhigh_pole(9)=mb_high_pole
c              else
c
c   use poles and zeros
c
...
c             Filt=9
c
c  use filters. disabled for now, gain is missing
c
c              if(ms_high_pole.gt.0.or.ms_low_pole.gt.0) then
c                 flow(9)=ms_low
c                 flow_pole(9)=ms_low_pole
c                 fhigh(9)= ms_high
c                 fhigh_pole(9)=ms_high_pole
c              else
c
c   use poles and zeros
c
...
c   however mss and mbb uses filters
c
...
c  the number of poles is set to 8, could have been 4
c
...
c                                                                               
       if(filt.ne.0) then
          text=' '                                                       
          write(text(1:19),201) flow(filt),fhigh(filt)                         
 201      format('Filt: ',f6.3,1x,f6.3)                                                                                                                                                   
          call xset_color(xred)      
          call xchars(text,19,xpos+600.0,max_ypos+3.0) 	! WCC moved down, JH LEFT
          call xset_color(color_def)
       endif                                      
c
c   put menu box if in graphics screen mode, set input mouse poistion outsde
c   ranfe so no choise is made (0.0,999.0)
c
       if(plotoption.lt.2)
...
c
c   make sure ratation is plotted
c
...
c
c------------------------------------------------------------
c   check if choice is all component  mode indicated  y
c----------------------------------------------------------------
c
       if(char(ich).eq.'y') then
          choice='ALLC'
...
c   Write output file for extracting time windows in cont mode
      rewind(extunittmp)
      read(extunittmp,*)text,station_code
      read(extunittmp,*)text,component_code
      read(extunittmp,*)text,max_line
      read(extunittmp,*)text,line_time
      read(extunittmp,*)text,height
 997  read(extunittmp,*,end=996)text,dummyx,dummyy
c     if(text(1:3).eq.'lin')line_start_time=dummyx
      if(text(1:3).eq.'pag')line_start_time=dummyx
...
c     if(text(1:3).eq.'beg')dummyb=line_start_time-
c    +(max_line-i+1)*line_time+dummyx/1024*line_time
c     if(text(1:3).eq.'end') then
c     dummye=line_start_time-
c    +(max_line-i+1)*line_time+dummyx/1024*line_time-dummyb
